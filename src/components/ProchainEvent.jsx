...
function ProchainEvent() {
  const events = [
    ...
    { day: "16", month: "Nov", title: "Le soufisme : une voie d'introspection", href: "/pageEvenementAlaUne", isLarge: true }
  ];

  const EventCard = ({ day, month, title, href, isLarge }) => (
    <a
      href={href || "#"}
      className={`flex grow p-8 my-4 mx-auto w-[100%] text-white bg-gray-900 rounded-3xl md:flex-wrap md:mt-4 md:max-w-full ${isLarge ? 'pb-12' : ''}`}
    ...
    >
      <div className="flex flex-auto gap-4 w-[90%] items-start">
        <div className="flex flex-col font-medium whitespace-nowrap">
          ...
          <div className="shrink-0 self-start w-10 h-0.5 bg-white" />
        </div>
        ...
        <img
          loading="lazy"
          src="https://cdn.builder.io/api/v1/image/assets/TEMP/622a48f5953e4efcbd93191a00c17ef1745e80759f04c985d0ec663268135304?apiKey=aa921c22d5e8470a9701bc1a79b29d5d&"
          alt=""
          className={`shrink-0 w-12 aspect-square ${isLarge ? 'self-end mt-4' : 'my-auto'}`}
        />
      </div>
    </a>
  );
